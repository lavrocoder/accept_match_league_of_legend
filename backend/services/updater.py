...
class ProjectUpdater:
    ...
    def get_update_url(self) -> str:
        """Получение URL для проверки обновлений"""
        config = self.load_config()
        return config['tool']['project-updater']['update_url']

    def get_critical_files(self) -> List[str]:
        """Получение списка критических файлов"""
        config = self.load_config()
        return config['tool']['project-updater'].get('critical_files', [])
    ...
    def apply_update(self, zip_path: Path) -> bool:
        ...
        try:
            ...
            critical_files = set(self.get_critical_files())
